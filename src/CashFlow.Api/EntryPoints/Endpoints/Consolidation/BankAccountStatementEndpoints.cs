...
using PocCQRS.Application.Queries.Statement;
using PocCQRS.Domain.Entities;
using PocCQRS.EntryPoint.EndPoints.Extensions;

namespace CashFlow.Api.EntryPoints.Endpoints.Consolidation;

public static class BankAccountStatementEndpoints
{
    public static void MapBankAccountStatementEndpoints(this WebApplication app)
    {
        app.MapGet("/api/v1/account/statement/{bankAccountId}",
            ...
            {
                var query = new BankStatementAllQuery(Guid.Parse(bankAccountId));
        ...
        app.MapGet("/api/v1/account/statement/{bankAccountId}/{days}",
            ...
            {
                var query = new BankStatementByDaysQuery(Guid.Parse(bankAccountId), days);
                var result = await mediator.Send(query);
        ...
        app.MapGet("/api/v1/account/statement/{bankAccountId}/inflow",
            async ([FromRoute] string bankAccountId, IMediator mediator) =>
            {
                var query = new BankStatementByOperationQuery(Guid.Parse(bankAccountId), OperationType: OperationType.Inflow);
                var result = await mediator.Send(query);
        ...
        app.MapGet("/api/v1/account/statement/{bankAccountId}/inflow/{days}",
            async ([FromRoute] string bankAccountId, int days, IMediator mediator) =>
            {
                var query = new BankStatementByDaysAndOperationQuery(Guid.Parse(bankAccountId), Days: days, OperationType: OperationType.Inflow);
                var result = await mediator.Send(query);
        ...
        app.MapGet("/api/v1/account/statement/{bankAccountId}/outflow",
            async ([FromRoute] string bankAccountId, IMediator mediator) =>
            {
                var query = new BankStatementByOperationQuery(Guid.Parse(bankAccountId), OperationType: OperationType.Outflow);
                var result = await mediator.Send(query);
        ...
        app.MapGet("/api/v1/account/statement/{bankAccountId}/outflow/{days}",
            async ([FromRoute] string bankAccountId, int days, IMediator mediator) =>
            {
                var query = new BankStatementByDaysAndOperationQuery(Guid.Parse(bankAccountId), Days: days, OperationType: OperationType.Outflow);
                var result = await mediator.Send(query);
